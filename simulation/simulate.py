...
# simulation parameters
# ------------------------------------------------------------------
DT          = 6.0 * 3600.0    # 6-hour timestep in seconds
N_MONTHS    = 264              # 2004-01 through 2025-12
KAPPA_V     = 1e-5             # m2/s, background diapycnal diffusivity
H1          = 200.0            # m, layer 1 thickness (fixed)
...
def apply_diffusion(C1, C2, grid, dt):
    ...
    Notes: the timescale kappa_v / (dz_grad * H1) is O(years-centuries),
    so exchange is slow and numerically stable at 6-hour steps.
    """
    H2     = np.maximum(grid['model_depth'] - H1, 0.0)
    active = (~grid['land_mask']) & (H2 > 0.0)

    dz_grad  = np.where(active, H1 / 2.0 + H2 / 2.0, 1.0)
    flux     = np.where(active, KAPPA_V * (C1 - C2) / dz_grad, 0.0)

    C1_new = C1 - flux / H1 * dt
    C2_new = C2 + np.where(H2 > 0.0, flux / H2 * dt, 0.0)
...
def load_stations(obs_csv, grid):
    """
    Unique PFOA station locations above 60N, snapped to nearest grid cell.
    Returns DataFrame: station_id, obs_lat, obs_lon, i, j
    """
    df = pd.read_csv(obs_csv)
    df = df[(df['compound'] == 'PFOA') & (df['lat'] >= 60.0)].copy()
    st = (df[['lat', 'lon']]
          ...
          .rename(columns={'lat': 'obs_lat', 'lon': 'obs_lon'}))
    st['station_id'] = st.index
    st['i'] = st['obs_lat'].apply(lambda x: int(np.argmin(np.abs(grid['lat'] - x))))
    st['j'] = st['obs_lon'].apply(lambda x: int(np.argmin(np.abs(grid['lon'] - x))))
    return st
...
def run():
    ...
    c_bnd_L1 = get_boundary_1d(grid, 100.0)   # layer 1: prescribed Atlantic/Pacific
    c_bnd_L2 = get_boundary_1d(grid, 250.0)   # layer 2: zeros

    # initialise concentration fields to zero
    C1 = np.zeros((grid['nlat'], grid['nlon']))
    C2 = np.zeros((grid['nlat'], grid['nlon']))

    # steps per month: 365.25/12 days * 86400 s/day / DT s/step
    steps_per_month = int(round((365.25 / 12.0 * 86400.0) / DT))
    print(f'steps per month: {steps_per_month}  ({steps_per_month * DT / 86400:.1f} days)')
    ...
    for t_idx in range(N_MONTHS):
        ...
        if month == 1:
            print(f'  {year}  (C1 max={C1.max():.4f} ng/L)')
        ...
        # source terms: fixed across all steps this month
        S = get_riverine_source(grid, year, month, riv_df, DT) + \
            get_atmospheric_source(grid, year, atm_df, DT)
...
if __name__ == '__main__':
    run()
>>>>>>> 701353d (created the full simulation)
